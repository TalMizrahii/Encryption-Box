...
def decipher_screen(root):
    ...
    def massage_validation(message):
        if len(message) <= 0:
            messagebox.showerror("MASSAGE ERROR", "Ciphertext must contain at least one letter!")
            return False
        elif not all(x.isalpha() or x.isspace() for x in message):
            messagebox.showerror("MASSAGE ERROR", "Ciphertext must contain letters and spaces only!")
            return False
        return True

    # Checking the key and ciphertext and sending it to be decrypted.
    def decipher_massage():
        ...
        if messagebox.askquestion("ATTENTION", "are you sure you entered the correct key?") == "yes":
            ...
            if key_validation(key, message) and massage_validation(message):
